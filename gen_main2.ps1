...
$mapSpec = @(
  @{map="Mirage"; desc="在Mirage中路拱门位置的控制让他成为队伍最可靠的支点"},
  @{map="Inferno"; desc="Inferno香蕉道的防守堪称教科书级别让突破手吃尽苦头"},
  @{map="Dust2"; desc="Dust2 A大门的单点突破经常为队伍打开局面"},
  @{map="Nuke"; desc="Nuke外场仓库区域的调度能力让对手难以预判"},
  @{map="Overpass"; desc="Overpass厕所区域的自由人打法独树一帜"},
  @{map="Ancient"; desc="Ancient甜甜圈位置的站位被多个分析师研究过"},
  @{map="Anubis"; desc="Anubis中路河道的控制力让对手不敢轻易过中"},
  @{map="Vertigo"; desc="Vertigo A坡争夺战中的发挥极其稳定让人放心"}
)
...
$countryStory["Turkey"] = @("伊斯坦布尔横跨欧亚的桥上也架起了CS梦想","土耳其电竞热潮中涌现的佼佼者","安卡拉CS网吧里常客如今站上世界舞台")
...
$countryStory["Canada"] = @("加拿大冰天雪地里CS是他唯一的温暖","枫叶之国的CS独苗在国际赛场发光","加拿大CS人才荒中的标杆性选手")
...
for ($i = 0; $i -lt $json.Count; $i++) {
  ...
  if ($facts) {
    foreach ($f in $facts) {
      $d = 3
      if ($hints.Count -gt 7) { $d = 4 }
      if ($hints.Count -gt 9) { $d = 5 }
      AddHint $f $d
    }
  }
  ...
  elseif ($name -match '^[A-Z]{3,}$') { AddHint "他的全大写ID在记分板上总是格外醒目" 3 }
  else { AddHint "ID $name 已成为CS职业圈一个辨识度很高的名字" 3 }
  
  # Team fact
  AddHint "$team 是他职业生涯最重要的舞台在这里他找到了属于自己的位置" 2
...
Write-Host "Written. Total:" $json.Count
Write-Host "Done!"