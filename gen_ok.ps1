...
for ($i = 0; $i -lt $json.Count; $i++) {
  ...
  AddHint $mapSpec[$idx % $mapSpec.Count].desc 2
  ...
  while ($hints.Count -lt 11) {
    ...
    AddHint $careerStories[$si] (if ($cc%3 -eq 0){3} elseif($cc%3 -eq 1){4} else{5})
    $cc++
  }
  ...
  # Age story
  AddHint $ageStories[[math]::Floor(($seed % 100) / 13) % $ageStories.Count] 4
  ...
  else { AddHint "ID $name 已成为CS职业圈辨识度很高的名字" 3 }
  ...
  while ($hints.Count -lt 10) {
    AddHint $careerStories[(($seed * 17 + $hints.Count) % $careerStories.Count)] 5
  }
...
[System.IO.File]::WriteAllText($outPath, $out, $utf8enc)
